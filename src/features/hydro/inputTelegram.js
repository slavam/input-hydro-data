...
export const InputHydroTelegram = ({ postCode, observDate }) => {
  ...
  const showGroup15 = () => {
    ...
    let g5 = ` 51201${g152}${g153}${g154}${g155}`
  ...
  const group5Jsx = (id, ipChange, iiChange) => {
    let isIntens = icePhenomenaWithIntens.indexOf(+ipChar[+id[3] - 1]) >= 0
    let label = isIntens ? 'Выберите характеристику или интенсивность явления' : 'Выберите характеристику явления'
    let options = isIntens ? Object.keys(icePhenomena).map(ip => <option key={ip} value={ip}>{icePhenomena[ip]}</option>) : Object.keys(icePhenomena).map(ip => { return (+ip > 10 ? <option key={ip} value={ip}>{icePhenomena[ip]}</option> : null) })
    return (<Form.Group className="mb-3" >
      <Form.Label>Выберите характеристику явления</Form.Label>
      <Form.Select id={id + 'ip'} onChange={ipChange} menuportaltarget={document.body} value={+ipChar[+id[3] - 1]}>
        {Object.keys(icePhenomena).map(ip => { return (+ip > 10 ? <option key={ip} value={ip}>{icePhenomena[ip]}</option> : null) })}
      </Form.Select>
      <Form.Label>{label}</Form.Label>
      <Form.Select id={id + 'ipi'} onChange={iiChange} value={+ipAddon[+id[3] - 1]}>
        {options}
      </Form.Select>
    </Form.Group>)
  }
  const ip1CodeChanged = e => {
    ...
    ipChar[i] = ip
    if (icePhenomenaWithIntens.indexOf(+ip) >= 0)
      ipAddon[i] = '01'
    else
      ipAddon[i] = '11'
    // console.log(ipAddon)
